...
int32 IO_TransSelectInput(IO_TransSelect_t *pSet, int32 timeoutUSec)
{
    ...
    CFE_PSP_MemCpy((void *)&pSet->fdSetActive, (void *)&pSet->fdSetFull, sizeof(fd_set));
    ...
    else
    {
        ...
        size            = select(pSet->iMaxFdNum + 1, &pSet->fdSetActive, NULL, NULL, &timeout);
    }
...
int32 IO_TransSelectOutput(IO_TransSelect_t *pSet, int32 timeoutUSec)
{
    ...
    else
    {
        timeout.tv_sec  = timeoutUSec / 1000000;
        timeout.tv_usec = timeoutUSec % 1000000;
        size            = select(pSet->iMaxFdNum + 1, NULL, &pSet->fdSetActive, NULL, &timeout);
    }
