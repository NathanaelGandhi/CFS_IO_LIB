...
int32 TC_SYNC_GetTransferFrame(uint8 *pTfBuff, uint8 *pCltu, uint16 tfBuffSize, uint16 cltuLength, boolean derandomize)
{
    ...
    while (1)
    {
        iStatus = TC_SYNC_GetCodeBlockData(pTfBuff, pCltuCursor, &tfOffset, &cltuOffset, tfBuffSize, cltuLength);
                               ...
                               uint16 tfBuffSize, uint16 cltuMaxLength)
{
    ...
    if (pTfBuff == NULL || pCodeBlock == NULL || pTfOffset == NULL || pCltuOffset == NULL)
    {
        iStatus = TC_SYNC_INVALID_POINTER;
        goto end_of_function;
    }
    ...
    CFE_PSP_MemCpy((void *)&pTfBuff[*pTfOffset], (void *)pCodeBlock, TC_SYNC_CODE_BLOCK_DATA_SIZE);
