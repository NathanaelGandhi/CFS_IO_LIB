...
int32 IO_TransUdpRcv(IO_TransUdp_t *udp, uint8 *buffer, int32 bufSize)
{
    ...
    msgSize = recvfrom(udp->sockId, (void *)buffer, (size_t)bufSize, 0, (struct sockaddr *)&udp->srcAddr, &addrLen);
...
int32 IO_TransUdpSnd(IO_TransUdp_t *udp, uint8 *msgPtr, int32 size)
{
    ...
    sizeOut = sendto(udp->sockId, (void *)msgPtr, (size_t)size, 0, (struct sockaddr *)&udp->destAddr,
