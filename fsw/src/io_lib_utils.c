...
const static uint8 revBitsTbl[16] = {0x0, 0x8, 0x4, 0xc, 0x2, 0xa, 0x6, 0xe, 0x1, 0x9, 0x5, 0xd, 0x3, 0xb, 0x7, 0xf};
...
int32 IO_LIB_UTIL_PseudoRandomize(uint8 *pBuff, uint16 buffSize, uint8 *pPrSeq)
{
    ...
    for (byte = 0; byte < buffSize; ++byte)
    {
        ...
        pBuff[byte] ^= (pPrSeq[prSeqIdx] << bitOffset) | (pPrSeq[(prSeqIdx + 1) % 32] >> (8 - bitOffset));
    }
